...
app.get("/", (req, res) => res.send("LiveZilla Platinum v9.4 - Ultimate Engine Active 🛡️🚀"));

// --- LOGGING ---
async function logToDB(event, session_id, data) {
    try {
        await supabase.from("events").insert([{ session_id, event, data }]);
...
app.post("/auth/login", async (req, res) => {
    ...
    try {
        ...
        if (rows.length > 0) {
            await logToDB("admin_login_success", "SYSTEM", { username });
            res.json({ status: "success", user: rows[0] });
        } else {
            await logToDB("admin_login_failed", "SYSTEM", { username });
            res.status(401).json({ status: "error", message: "Login ou senha incorretos" });
        }
    } catch (e) { 
        await logToDB("admin_login_error", "SYSTEM", { error: e.message });
...
app.post("/auth/forgot-password", async (req, res) => {
    ...
    try {
        ...
        if (rows.length > 0) {
            ...
            await logToDB("password_recovery_requested", "SYSTEM", { email });
        }
        res.json({ status: "ok" });
